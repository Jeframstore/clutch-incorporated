...
async function loadTermsContent() {
    try {
        const snap = await database.ref('settings/terms').once('value');
        let termsHTML = snap.val();
        ...
        if (!termsHTML) {
            ...
            `;
            await database.ref('settings/terms').set(termsHTML);
        }
        ...
        console.error('Error loading terms content:', e);
    }
}
